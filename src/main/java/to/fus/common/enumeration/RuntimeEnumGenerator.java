...
public final class RuntimeEnumGenerator {
     ...
     *
     * @param clazz T extends EnumCode<?>
    ...
    private static Object makeEnum(Class<?> enumClass, String value, int ordinal, Class<?>[] additionalTypes,
        ...
        if (constructor == null) {
            throw new IllegalArgumentException(
                    "Impossibile ricavare un costruttore per generare un nuovo valore nell'Enum "
    ...
    private static Constructor<?> getConstructor(Class<?> enumClass, Class<?>[] additionalParameterTypes) {
        ...
        for (Constructor<?> constr : enumClass.getDeclaredConstructors()) {
            if ((minNumParamConstr == null || (minNumParamConstr.getParameterTypes().length
                    > constr.getParameterTypes().length))// confronto
                    // il numero
                    // dei
                    // parametri
    ...
    private static boolean checkConstructorParameters(Constructor<?> constr) {
        ...
        return (constr.getParameterTypes().length >= 2) // deve contenere almeno due parametri
                && (String.class.equals(constr.getParameterTypes()[0]) && int.class.equals(
                constr.getParameterTypes()[1])) // controllo che i due parametri siano
